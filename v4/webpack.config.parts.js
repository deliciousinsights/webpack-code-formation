// Webpack v4 - Éléments de configuration
// ======================================
...
// -----
//
// Passage des fichiers `.js` et `.jsx` par Babel, par défaut en preset `env`
// sans transpilation des modules (pour que Webpack puisse faire ses optims)
// mais avec `useBuiltIns: true` (pour minimiser l’impact éventuel de
// `babel-polyfill`). On ignore aussi `node_modules` par défaut.
exports.babelize = ({
  include,
...
// Le fichier produit inclue automatiquement un hash pour le *Long-Term
// Caching*.
exports.extractCSS = ({ include, exclude, modules }) =>
  extractStyling({ ext: 'css', include, exclude, modules })

// Idem, mais pour les sources `.scss`.  Préfixe la pipeline de chargeurs par le
// transpileur SASS.
...
// --------------
//
// Charge les fontes en préférant un *inlining* en-dessous de 10Ko, sous forme
// d’URL `data:` en Base64.  Même si on teste ici toutes les extensions usuelles
// de *web fonts*, en pratique WOFF + WOFF2 suffit généralement à couvrir tous
// les navigateurs.  Les fichiers générés sont automatiquement hashés, par
// précaution à 16 caractères (8 suffiraient sans doute).
...
// Charge les images en préférant un *inlining* en-dessous de 10Ko, sous forme
// d’URL `data:` en Base64 (pour les images *raster*, basées pixels) ou UTF-8
...
// retravaillées pour passer correctement sur cette !@# d’IE.
...
// comprendra les bonnes balises `<script>` (et éventuellement `<link
// rel="stylesheet"/>`) en fonction des bundles définis dans la configuration.
// Peut être appelé plusieurs fois avec ses [moult
// options](https://github.com/webpack-contrib/html-webpack-plugin#readme) pour
...
// Ne bundle pas par défaut les locales de Moment.js (~50Ko min+gz quand
...
exports.minifyAll = () => {
  // Terser est la version à jour d’uglify-es, qui a été abandonné.  Exige WP4.
  const TerserPlugin = require('terser-webpack-plugin')
  // Pour minifier du CSS (basé cssnano, ça ça marche bien)
...
// amplement suffisant pour 99,9% des cas.
//
// Si vous spritez les images, assurez-vous de faire passer cette optimisation
// *après* le spriting, pas avant (donc *avant* en termes de pipeline de
// chargeurs…).
...
// Purge le dossier de build (`output.path`), ce qui est notamment utile quand
...
// Injecte le « serveur » de Webpack-Dashboard, pour que la CLI de celui-ci
// puisse s’interfacer avec le moteur de Webpack.
exports.dashboard = (options) => {
  const WebpackDashboardPlugin = require('webpack-dashboard/plugin')
  return { plugins: [new WebpackDashboardPlugin(options)] }
}

// Personnalisation du serveur de webpack-Dev-Server.  On est en mode `hot` par
// défaut (`hot`, pas `hotOnly`), avec dans ce cas le plugin HMR injecté ainsi
...
// aussi de base le *deep linking* (option `historyApiFallback` à `true`) :
// toute URL non reconnue, hors proxying éventuel, renverra le `/index.html`.
//
// Par rapport aux options de base du bloc `devServer`, on ajoute un peu de
// confort :
//
// - Au lieu d’options booléennes séparées `hot` et `hotOnly`, on a `hot` qui
//   peut valoir `true`, `false` ou `'only'`.
// - Au lieu d’options séparées `open` et `openPage`, on a une option `open` qui
//   peut valoir `true`, `false` ou l’URL à ouvrir.
exports.devServer = ({
  contentBase,
  hot = true,
  https,
...
// Overlay d’erreur amélioré pour les erreurs à l'exécution au sein de la page.
...
// Empêche Webpack de produire des assets (en mémoire ou sur disque) lorsqu’1+
// asset rencontre une erreur.  Permet d’éviter les builds « le cul entre deux
// chaises ».  Automatique en production dans Webpack 4.
exports.safeAssets = () => ({
  plugins: [new webpack.NoEmitOnErrorsPlugin()],
})

// Active le plugin `HardDisk`, qui cache de façon persistente la plupart des
// étapes intermédiaires de traitement de modules par les chargeurs et les
// plugins, au moyen du mécanisme Webpack de *records*.  Sans doute l’opti la
...
// Construit une pipeline de chargeurs CSS, avec ou sans `style-loader` en fin
...
// `altLang` est fourni (ex. `sass`, `stylus`, `less`).  Cœur de génération des
// règles pour les extracteurs (prod) et injecteurs (dev) de style.
//
// L’option `modules`, si elle est juste booléenne, se transforme en une série
// d’options fines pour de meilleures pratiques : export *camel-case only* et
// construction plus « débogable » des noms de classes dynamiques.
...
// Construction générique d’une pipeline d’extraction CSS.  Se repose en interne
// sur `buildCSSRule(…)`.
...
// Construction générique d’une pipeline d’injection CSS.  Se repose en interne
// sur `buildCSSRule(…)`.
